...
def consistent(assignment, csp, var, value):
    ...
    for constraint in csp.binaryConstraints:
        if constraint.affects(var):
            ...
            if neighbour in assignment.assignedValues.keys() and not constraint.isSatisfied(value, assignment.assignedValues[neighbour]):
                return False
    return True
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
    ...
    if var == None:
        return assignment
    domain = orderValuesMethod(assignment, csp, var)
    for val in domain:
...
def minimumRemainingValuesHeuristic(assignment, csp):
    ...
    # TODO: Question 2
    varSortDict = {}
    for each in domains.items():
        if not assignment.isAssigned(each[0]):
            varSortDict[each[0]] = (len(each[1]), -1 * degreeHeuristic(assignment, csp, each[0]))
    varSortList = sorted(varSortDict.items(), key = lambda dict:dict[1])
    if len(varSortDict):
        nextVar = varSortList[0][0]
    return nextVar
    raise_undefined_error()
...
def degreeHeuristic(assignment, csp, var):
    ...
    for constraint in csp.binaryConstraints:
        if constraint.affects(var):
            neighbour = constraint.otherVariable(var)
            if assignment.assignedValues[neighbour] == None:
                degree += 1
    return degree
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
    ...
    for val in domain:
        constraints = 0
        assignment.assignedValues[var] = val
        for each in csp.varDomains:
            if each != var:
                for leftVal in assignment.varDomains[var]:
                    if consistent(assignment, csp, var, leftVal):
                        constraints += 1
        possibleValDict[val] = constraints
        assignment.assignedValues[var] = None
    if len(possibleValDict):
        possibleValList = sorted(possibleValDict.items(), key = lambda dict:dict[1], reverse = True)
        for each in possibleValList:
...
def forwardChecking(assignment, csp, var, value):
    ...
    for constraint in csp.binaryConstraints:
        if constraint.affects(var):
            neighbour = constraint.otherVariable(var)
            if not assignment.isAssigned(neighbour):
                removeValSet = set([])
                for val in domains[neighbour]:
                    if not consistent(assignment, csp, neighbour, val):
                        inferences.add((neighbour, val))
                        removeValSet.add(val)
                if removeValSet == domains[neighbour]:
                    return None
    for each in inferences:
...
def revise(assignment, csp, var1, var2, constraint):
    ...
    for val1 in assignment.varDomains[var1]:
        ...
        if ifRemove:
            ...
            removeValSet.add(val1)
    if removeValSet == assignment.varDomains[var1]:
        return None
    return inferences
...
def maintainArcConsistency(assignment, csp, var, value):
    ...
    while(True):
        if len(reviseQueue) == 0:
            break
        vars = reviseQueue.pop()
        for constraint in csp.binaryConstraints:
            if constraint.affects(vars[0]) and constraint.affects(vars[1]):
                if not assignment.isAssigned(vars[1]):
                    revisedInference = revise(assignment, csp, vars[0], vars[1], constraint)
                    if revisedInference == None:
                        for each in inferences:
                            assignment.varDomains[each[0]].add[each[1]]
                        return None
                    if len(revisedInference):
                        for each in csp.binaryConstraints:
                            if each.affects(vars[1]):
                                reviseQueue.appendleft((vars[1], each.otherVariable(vars[1])))
                        for each in revisedInference:
                            inferences.add(each)
                            assignment.varDomains[each[0]].discard(each[1])
    return inferences
...
def AC3(assignment, csp):
    ...
    while(True):
        ...
        for constraint in csp.binaryConstraints:
            ...
            if len(revisedInference):
                for constraint in csp.binaryConstraints:
                    if constraint.affects(vars[1]):
                        reviseQueue.appendleft((vars[1], constraint.otherVariable(vars[1])))
                for each in revisedInference:
                    inferences.add(each)
                    assignment.varDomains[each[0]].discard(each[1])
    return assignment
